...
\item See \hyperref[pdf:imgcom]{code at end} or the . Here's the Frobenius norms:
\begin{align*}
...
\item A comparison has been shown in \cref{fig:figr}. At first glance, one cannot compare at all. Only when one looks carefully, we can see the minor difference: towards the center of Conway's hair, a couple of hair strands can be seen in the original picture, which are `dissolved' in $A_{{(160)}}$.
\end{enumerate}
